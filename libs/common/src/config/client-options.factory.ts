...
export function createMicroserviceOptions(
    ...
    const transport: Transport | undefined =
        Transport[transportStr as keyof typeof Transport];
    ...
    switch (transport) {
        case Transport.TCP:
            ...
            if (!options.port)
                throw new Error(`Missing ${prefix}_PORT for ${transportStr}`);
            break;
    ...
    if (!options.port && !options.urls) {
        logger.error(
            `Failed to configure client for ${definition.token}. Missing critical options.`,
        );
        throw new Error(
            `Configuration error for microservice client: ${definition.token}`,
        );
    }
...
export function createClientsAsyncOptions(
    ...
    return {
        ...
        useFactory: (configService: ConfigService) => {
            ...
            const transportStr = configService
                .get<string>(`${prefix}_TRANSPORT`, 'TCP')
                .toUpperCase();
            const transport: Transport | undefined =
                Transport[transportStr as keyof typeof Transport];
            if (transport === undefined) {
            ...
            switch (transport) {
                case Transport.TCP:
                    options.host = configService.get<string>(
                        `${prefix}_HOST`,
                        'localhost',
                    );
                    ...
                    if (!options.port)
                        throw new Error(
                            `Missing ${prefix}_PORT for ${transportStr}`,
                        );
                    break;
